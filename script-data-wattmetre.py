...
def find_index2(index1: int, heure2: str, times: [str]) -> int:
    ...
    if heure2!='None':
        ...
        for (heure, index) in zip(times[debut:], range(debut,len(times)-debut)): 
...
def parse_annotation(annotations: str)-> dict:
    return json.loads(annotations.replace("'", '"'))

def plot(x:[str],y:[int],labels:[str],factor_labels:int,factor:int,annotations:[], title:str):

    
    ############## plot  ################################

    plt.plot(x, y, color = 'g', linestyle = 'dashed', marker = 'o', label = "données wattmètre")

    if annotations:
        dict_ann=parse_annotation(annotations)
        for heure in dict_ann: # TODO 
...
def parser_et_run():
    ...
    parser.add_argument('--hour1', type=str, nargs='?', const='None', default='None',
                        help='filter the file between specific hours; default none')
    ...
    parser.add_argument('-a','--annotations',nargs='?',
                        help="annotate an event : json format \"{'11:22:00':'coucou','11:23:00':'salut'}\"")
    ...
    plot(x,y,labels,factor_labels,factor,annotations,title)
